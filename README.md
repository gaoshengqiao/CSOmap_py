...
## Features of this Python version

- **No .mat files**: All intermediate data is saved as Python `.pkl` (pickle) files.
- **Label-safe**: Cell and gene names are strictly handled as strings to avoid numeric/auto-cast mismatches between `TPM.txt` and `label.txt`.
- **Header-aware**: `label.txt` parser automatically skips common header rows (e.g. `cells\tlabels`, `Cell\tLabel`).
- **Text output**: `coordinate.txt` exports text labels directly (not numeric cluster IDs).
